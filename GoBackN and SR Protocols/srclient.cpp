...
int main(int argc, char const *argv[]) { 
	...
	if(argv[1][0] == '-' && argv[1][1] == 'd')
	{
		...
		debugflag = true;
		seqbits = atoi(argv[7]);
		packetlength = atoi(argv[9]);
		packetgenrate = atoi(argv[11]);
		maxbuffersize = atoi(argv[17]);		
		MAX_PACKETS = atoi(argv[13]);
		windowsize = atoi(argv[15]);
	}
	else
	{
		//debugflag = true;
		seqbits = atoi(argv[6]);
		packetlength = atoi(argv[8]);
		packetgenrate = atoi(argv[10]);
		maxbuffersize = atoi(argv[16]);		
		MAX_PACKETS = atoi(argv[12]);
		windowsize = atoi(argv[14]);
	}
	...
	while(lastack < MAX_PACKETS-1)
	{
		...
		while(g--)
		{
			...
			if(lastack-g+windowsize < MAX_PACKETS && acked[lastack-g+windowsize] != 1)
			{
				// int e = maxbuffersize;
		...
		while(g--)
		{
			...
			//if(nom != lastack){
				if(nim != -1){
					...
					if(debugflag == true)
					{
						cout << "Seq No "<<(nom % ((1 << seqbits)-1)) << "; time generated : "
						<< a[nom] << "; rtt : " << rtt[nom]<< " ; number of attempts"<< sentcount[nom] << endl;
						;
					}

					while(lastack != MAX_PACKETS-1 && acked[lastack+1] == 1)
					{
						lastack++;
					}
					cout << "lastack moved to " << lastack << endl;
					// while(++r<= lastack)
					// {
					// 	pthread_mutex_lock(&lockt);
					// 	for(int i = 0; i < maxbuffersize; i++)
